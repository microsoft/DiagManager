...
{
    public class EventField : DiagItem, IComparable<EventField>
    {
        ...
        {
            return string.Format("{0}:{1}:{2}", Name, AutoInclude, IsNum);
        }

        public int CompareTo(EventField other)
        {
            if (null == other) return 1;
            return this.Name.CompareTo(other.Name);
        }
        public EventField(string name, bool autoinc, bool isnum)
        {
            this.Name = name;
            this.AutoInclude = autoinc;
            this.IsNum = isnum;
        }
        public EventField(string EventFieldElement)
        {
            ...
            while (iterEventField.MoveNext())
            {
                ...
                this.AutoInclude = Convert.ToBoolean(eventNav.GetAttribute("AutoInclude", ""));
                this.IsNum = Convert.ToBoolean(eventNav.GetAttribute("IsNum", ""));
            ...
            if (counter != 1)
            {
                throw new ArgumentException("The XML passed for Event Field is not valid and should only contain one record");
            }
        }
    } //
}
