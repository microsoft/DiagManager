...
sql_collect_perfstats()
{
        if [[ $COLLECT_PERFSTATS == [Yy][eE][sS] ]] ; then
                ...
                `/opt/mssql-tools*/bin/sqlcmd -S$SQL_SERVER_NAME $CONN_AUTH_OPTIONS -C -i"SQL_Perf_Stats.sql" -o"$outputdir/${1}_${2}_SQL_Perf_Stats_Output.out"` &
                mypid=$!
                #printf "%s\n" "$mypid" >> $outputdir/pssdiag_stoppids_sql_collectors.txt
		sleep 5s
                pgrep -P $mypid  >> $outputdir/pssdiag_stoppids_sql_collectors.txt
                `/opt/mssql-tools*/bin/sqlcmd -S$SQL_SERVER_NAME $CONN_AUTH_OPTIONS -C -i"SQL_DMV_Snapshots.sql" -o"$outputdir/${1}_${2}_SQL_DMV_Snapshots.out"` &
                mypid=$!
                #printf "%s\n" "$mypid" >> $outputdir/pssdiag_stoppids_sql_collectors.txt
		sleep 5s
                pgrep -P $mypid  >> $outputdir/pssdiag_stoppids_sql_collectors.txt
...
sql_collect_counters()
{
        if [[ $COLLECT_SQL_COUNTERS == [Yy][eE][sS] ]] ; then
                ...
                `/opt/mssql-tools*/bin/sqlcmd -S$SQL_SERVER_NAME $CONN_AUTH_OPTIONS -C -i"SQL_Performance_Counters.sql" -o"$outputdir/${1}_${2}_SQL_Performance_Counters.out"` &
                mypid=$!
                #printf "%s\n" "$mypid" >> $outputdir/pssdiag_stoppids_sql_collectors.txt
		sleep 5s
                pgrep -P $mypid  >> $outputdir/pssdiag_stoppids_sql_collectors.txt
...
sql_collect_memstats()
{
        if [[ $COLLECT_SQL_MEM_STATS == [Yy][eE][sS] ]] ; then
                ...
                `/opt/mssql-tools*/bin/sqlcmd -S$SQL_SERVER_NAME $CONN_AUTH_OPTIONS -C -i"SQL_Mem_Stats.sql" -o"$outputdir/${1}_${2}_SQL_Mem_Stats_Output.out"` &
                mypid=$!
                #printf "%s\n" "$mypid" >> $outputdir/pssdiag_stoppids_sql_collectors.txt
		sleep 5s
                pgrep -P $mypid  >> $outputdir/pssdiag_stoppids_sql_collectors.txt
...
sql_collect_custom()
{
        if [[ $CUSTOM_COLLECTOR == [Yy][eE][sS] ]] ; then
                ...
                for filename in my_custom_collector*.sql; do
                   `/opt/mssql-tools*/bin/sqlcmd -S$SQL_SERVER_NAME $CONN_AUTH_OPTIONS -C -i"${filename}" -o"$outputdir/${1}_${2}_${filename}_Output.out"` &
                    mypid=$!
                    #printf "%s\n" "$mypid" >> $outputdir/pssdiag_stoppids_sql_collectors.txt
		    sleep 5s
                    pgrep -P $mypid  >> $outputdir/pssdiag_stoppids_sql_collectors.txt
...
sql_collect_xevent()
{
        ...
        if [[ $COLLECT_EXTENDED_EVENTS == [Yy][eE][sS]  ]]; then
                ...
                /opt/mssql-tools*/bin/sqlcmd -S$SQL_SERVER_NAME $CONN_AUTH_OPTIONS -C -i"${EXTENDED_EVENT_TEMPLATE}.sql" -o"$outputdir/${1}_${2}_pssdiag_xevent.out"
		...
		if [[ "$2" == "host_instance" ]] || [[ "$2" == "instance" ]]; then
                ...
                /opt/mssql-tools*/bin/sqlcmd -S$SQL_SERVER_NAME $CONN_AUTH_OPTIONS -C -i"pssdiag_xevent_start.sql" -o"$outputdir/${1}_${2}_pssdiag_xevent_start.out"
        fi
}
...
sql_collect_trace()
{
		...
		/opt/mssql-tools*/bin/sqlcmd -S$SQL_SERVER_NAME $CONN_AUTH_OPTIONS -C -i"MSDiagProcs.sql" -o"$outputdir/${1}_${2}_MSDiagprocs.out"
		...
		/opt/mssql-tools*/bin/sqlcmd -S$SQL_SERVER_NAME $CONN_AUTH_OPTIONS -C -i"pssdiag_trace_start.sql" -o"$outputdir/${1}_${2}_pssdiag_trace_start.out"
        fi
}
...
sql_collect_config()
{
        ...
        /opt/mssql-tools*/bin/sqlcmd -S$SQL_SERVER_NAME $CONN_AUTH_OPTIONS -C -i"SQL_Configuration.sql" -o"$outputdir/${1}_${2}_SQL_Configuration_Startup.out"
}

sql_collect_pal_DMVs()
{
        echo -e "$(date -u +"%T %D") Collecting PAL DMVs Snapshot at Startup..." | tee -a $pssdiag_log
        /opt/mssql-tools*/bin/sqlcmd -S$SQL_SERVER_NAME $CONN_AUTH_OPTIONS -C -i"SQL_DMV_PAL_Snapshots.sql" -o"$outputdir/${1}_${2}_SQL_DMV_PAL_Snapshots_Startup.out"
}

sql_collect_perfstats_snapshot()
{
        echo -e "$(date -u +"%T %D") Collecting Perf Stats Snapshot at Startup..." | tee -a $pssdiag_log
        /opt/mssql-tools*/bin/sqlcmd -S$SQL_SERVER_NAME $CONN_AUTH_OPTIONS -C -i"SQL_Perf_Stats_Snapshot.sql" -o"$outputdir/${1}_${2}_SQL_Perf_Stats_Snapshot_Startup.out"
}
